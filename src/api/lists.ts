...
export class ListsKlaviyoApi {
  ...
  // See https://a.klaviyo.com/api/v2/list/{list_id}/members for details
  public async addMembersToList(listId: string, profiles: Partial<KlaviyoProfile>): Promise<void> {
    ...
    if (res.ok) {
      ...
      await waitForRetry(res);
      return await this.addMembersToList(listId, profiles);
    } else {
      throw new KlaviyoError(res);
    }
  }
