...
export class ProfilesKlaviyoApi {
  ...
  // See https://developers.klaviyo.com/en/reference/get-profile-id for details
  public async getProfileId(identifier: string, identifierType: KlaviyoIdentifierType): Promise<KlaviyoProfile['id']> {
    const url = `https://a.klaviyo.com/api/v2/people/search?${identifierType}=${identifier}&api_key=${encodeURI(
      this.apiKey,
    )}`;
    ...
    if (res.ok) {
      return (await res.json()) as KlaviyoProfile['id'];
    } else if (res.status === 404) {
      return null;
    } else if (res.status === 429) {
      await waitForRetry(res);
      return await this.getProfileId(identifier, identifierType);
    } else {
      throw new KlaviyoError(res);
    }
  }
